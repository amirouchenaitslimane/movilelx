...
?>
          <ul class="nav nav-tabs mt-3" role="tablist">
            <li class="nav-item">
              <a class="nav-link" id="ped" href="#pedidos" role="tab" data-toggle="tab">pedidos
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link " href="#profile" role="tab" data-toggle="tab">Mi perfil
              </a>
            </li>
          </ul>
            ...
            <div role="tabpanel" class="tab-pane fade in  " id="pedidos">
              <div class="card">
                <div class="card-header">
                  <h4 class="header-title">Mis Pedidos:
                  </h4>
                </div>
                <div class="card-body">
                  <div class="row">
                    <div class="col-md-6 col-12 col-sm-6 " id="user">
                      <p   class="text-primary">Nombre: 
                        <strong id="nombre" class="text-dark">
                          <?= htmlspecialchars(trim($user->getNombre())) ;?>
                        </strong>
                      </p>
                      <p class="text-primary" >Apeilldo: 
                    ...
                    <div class="col-md-6 col-12 col-sm-6">
                      <p class="text-primary">Direccion: 
                        <strong class="text-dark">
                          <?= $user->getDireccion(); ?>
                        </strong>
                      </p>
                      <p class="text-primary">Cuenta registrada desde: 
                        <strong class="text-dark">
                          <?= $user->getCreated()?>
                        </strong>
                      </p>
                  ...
                  <div id="accordion" >
                    <div class="card " id="acco-<?= $pedido->getId();?>">
                      <div class="card-header " id="<?= $pedido->getId() ?>">
                        <h5 class="mb-0">
                          <div data-toggle="collapse" data-target="#col<?= $pedido->getId() ?>" aria-expanded="true" aria-controls="collapseOne">
                            <p class="pull-left text-success lead">Pedido n=º 
                              <?= $pedido->getId() ?>
                            </p>
                            <p class="pull-right ">creado el dìa 
                              <?= $pedido->getFecha() ?>
                            </p>
                          </div>
                        </h5>
                      ...
                      <div id="col<?= $pedido->getId() ?>" class="collapse show" aria-labelledby="<?= $pedido->getId() ?>" data-parent="#accordion">
                        <div class="card-body">
                          <h5>
                            <strong class="text-info bg-<?= $pedido->getEstado() ?>">Estado del pedido  
                            </strong> 
                            <?=\app\Pedido::getEstadoOption()[$pedido->getEstado()]?>
                          </h5>
                          <table class="table">
                            <thead>
                              <th scope="col">N producto 
                              </th>
                              <th scope="col">p/u
                              </th>
                              <th scope="col">Unidades
                              </th>
                              <th scope="col">P compra
                              </th>
                            </thead>
                            <tbody>
                              <?php
foreach ($linped_manager->getLinaPedidoPedido($pedido->getId()) as $linea):
?>
                              <tr>
                                <td>
                                  <?= htmlspecialchars(trim($linea->nombre)) ?>
                                </td>
                                <td>
                                  <?= htmlspecialchars(trim($linea->precio)) ?>€
                                </td>
                                <td>
                                  <?= htmlspecialchars(trim($linea->cantidad)) ?>
                                </td>
                                <td class="pull-right">
                                  <?= htmlspecialchars(trim($linea->precio_compra)) ?> €
                                </td>
                              </tr>
                              <?php
endforeach;
?>
                              <tr>
                                ...
                                <td class="pull-right" align="right">
                                  <strong>
                                    <?= trim($linped_manager->TotalPrecioCompra($pedido->getId())->total) ?> €
                                  </strong>
                                </td>
                              </tr>
                              <tr>
                                ...
                                <td class="pull-right" align="right">
                                  <strong>
                                    <?= trim($linped_manager->TotalPrecioCompra($pedido->getId())->total*\app\Cart::_IVA) ?> €
                                  </strong>
                                </td>
                              </tr>
                            </tbody>
                            <tfoot>
                              <tr>
                                ...
                                <td class="pull-right" align="right">
                                  <strong>
                                    <?= (trim($linped_manager->TotalPrecioCompra($pedido->getId())->total) + ($linped_manager->TotalPrecioCompra($pedido->getId())->total * \app\Cart::_IVA)); ?> €
                                  </strong>
                                </td>
                              </tr>
                            </tfoot>
                          </table>
                          <small class="text-muted">(n Producto *) Nombre del producto, (p/u *) precio unitario ,(unidades *) cantidad pedida,(p.compra *) precio compra 
                          </small>																												
                          <hr>
                          <button class="download lead text-primary btn btn-light" id="download-<?= $pedido->getId(); ?>  ignore" onclick="download_DIVPdf(<?= $pedido->getId() ?>)">Descargar el pedido >> 
                          </button>
                        </div>
                      </div>
                    </div> 	 	
            ...
            <div role="tabpanel" class="tab-pane fade" id="profile">
              ...
              <a href="actualizarperfile.php" class="btn btn-orange btn-success">Cambiar mis datos
              </a>
            </div>
          </div>
        </div>
...
<script>
  ...
  function download_DIVPdf(id) {
    ...
    pdf.text(140, 40, "amirouchenaitslimane@gmail.com -  (34) 966 91 22 60 ");
    var user = $("#user").html();
    pdf.setFontSize(9);
    ...
    var table = $("#acco-"+id).html();
    var htmlsource = user + table;
