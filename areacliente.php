																						...
																						<div class="card " id="acco-<?= $pedido->getId();?>">
																								...
																								<div id="col<?= $pedido->getId() ?>" class="collapse show" aria-labelledby="<?= $pedido->getId() ?>" data-parent="#accordion">
																										<div class="card-body">
																												...
																												<table class="table"><thead><tr><th scope="col">N producto </th><th scope="col">p/u</th><th scope="col">Unidades</th><th scope="col">P compra</th></tr></thead><tbody><?php foreach ($linped_manager->getLinaPedidoPedido($pedido->getId()) as $linea): ?><tr><td><?= htmlspecialchars(trim($linea->nombre)) ?></td><td><?= htmlspecialchars(trim($linea->precio)) ?>€</td><td><?= htmlspecialchars(trim($linea->cantidad)) ?></td><td><?= htmlspecialchars(trim($linea->precio_compra)) ?> €</td></tr><?php endforeach; ?><tr><td><strong>subtotal:</strong></td><td></td><td></td><td>  <strong><?= trim($linped_manager->TotalPrecioCompra($pedido->getId())->total) ?> €</strong></td></tr><tr><td><strong>IVA (<?= (\app\Cart::_IVA*100)?> %)</strong></td><td></td><td></td><td>  <strong><?= trim($linped_manager->TotalPrecioCompra($pedido->getId())->total*\app\Cart::_IVA) ?> €</strong></td></tr>    <tr><td><strong>subtotal:</strong></td><td></td><td></td><td class="text-warning">  <strong><?= (trim($linped_manager->TotalPrecioCompra($pedido->getId())->total) + ($linped_manager->TotalPrecioCompra($pedido->getId())->total * \app\Cart::_IVA)); ?> €</strong></td></tr></tbody></table>
																												<small class="text-muted">(n Producto *) Nombre del producto, (p/u *) precio unitario ,(unidades *) cantidad pedida,(p.compra *) precio compra </small>																												<hr>
