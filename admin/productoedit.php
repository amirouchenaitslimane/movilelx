...
if(isset($_POST['submit']))
{
	$lastImage = $producto->getImagen();
		...
		if (empty($producto->getErrors())){
				if($producto_manager->updateProduct($producto) === true){
            flash('info','producto actualizado con exito! ','success');
				}else{
            flash('info','ha occorrido un problema no hemos podido actualizar el producto intentalo mas tarde! ','danger');
				}
...
<?php foreach ($categoria_manager->displayCategorias() as $categoria): if(!empty($categoria->getChilds())|| $categoria->getPadreId()=='0'):?>
																				<optgroup label="<?= $categoria->getNombre() ?>">
<?php foreach ($categoria->getChilds() as $child):?>
																				<option value="<?= $child->getId();?>" <?= (($producto->getCategoriaId() == $child->getId()) ? "selected":"" ) ?> > <?= $child->getNombre().' ( '.$categoria->getNombre().' ) ';?> </option>
<?php endforeach; ?>
																				</optgroup>
                                            <?php else: ?>
